...
// copied from https://unpkg.com/robot3@0.4.1/dist/machine.js
...
let empty = () => ({});
let identity = (a) => a;
let callBoth = (par, fn, self, args) =>
  par.apply(self, args) && fn.apply(self, args);
let callForward = (par, fn, self, [a, b]) =>
...
let stateActionType = { type: "stateActionType" };
let stateAction = fnType.bind(stateActionType);
...
let stateType = { enter: identity, executeStateActions: identity };
function state(...args) {
  console.debug(`robot3: state: args = `, args);
  ...
  const stateActions = filter(stateActionType, args).map((t) => t.fn);
  if (stateActions.length) {
    console.debug(`robot3: state: stateActions = `, stateActions);
    let stateActionsStack = stack(stateActions, identity, callForward);
    desc.executeStateActions = valueEnumerable(stateActionsStack);
  }

  console.debug(`robot3: state: stateType = `, stateType, `, desc = `, desc);
  const result = create(stateType, desc);
  console.debug(`robot3: state: result = `, result);
  return result;
}
...
function transitionTo(service, machine, fromEvent, candidates) {
  ...
  for (let { to, guards, reducers } of candidates) {
    if (guards(context, fromEvent)) {
      ...
      console.debug(
        `robot3: transitionTo: calling state.enter... state = `,
        state
      );
      state.executeStateActions(newMachine, service, fromEvent);
...
function send(service, event) {
  console.debug(`robot3: send: service`, service, `, event = `, event);
...
function interpret(machine, onChange, initialContext, event) {
  ...
  s.machine.state.value.executeStateActions(s.machine, s, event);
