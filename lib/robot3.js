...
let entryType = { type: "entryType" };
let entry = fnType.bind(entryType);
...
let stateType = { enter: identity, entry: identity };
function state(...args) {
  ...
  const entries = filter(entryType, args).map((t) => t.fn);
  if (entries.length) {
    //console.debug(`robot3: state: entries = `, entries);
    let entriesStack = stack(entries, identity, callForward);
    desc.entry = valueEnumerable(entriesStack);
  }
...
function transitionTo(service, machine, fromEvent, candidates) {
  //console.debug(    `robot3: transitionTo: service = `,    service,    `, machine = `,    machine,    `, candidates = `,    candidates  );
  let { context } = service;
  for (let { to, guards, reducers } of candidates) {
    if (guards(context, fromEvent)) {
      ...
      //console.debug(        `robot3: transitionTo: calling state.enter... state = `,        state      );
      state.entry(newMachine, service, fromEvent);
...
function interpret(machine, onChange, initialContext, event) {
  ...
  s.machine.state.value.entry(s.machine, s, event);
